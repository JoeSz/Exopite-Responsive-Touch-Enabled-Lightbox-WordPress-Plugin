...
*/
  ...
  $(selector).find(insideElement).click(function(e) {
    // Do not activate lightbox if the link href not pointing to the image,
    // so follow link instead of open the image
    if ($(this).is("img")) {
    ...
    var slideNum = $(selector).find(insideElement).not('.lightbox35-no-gallery').index(this);
    size = $(selector).find(insideElement).not('.lightbox35-no-gallery').length;
    ...
    if ($('#lightbox35').length > 0) {
      ...
      function addSlide(item, index) {
        ...
        if ($(item).is("img")) {
          ...
          $('#lightbox35').append(frame_begin + frame_middle + '<img src="' + $href + '">' + caption + frame_end);

          // Add thumbnail to lightbox thumbnail
          if(!hideThumbnails) {
            $('#lightbox35 .thumbnails').append('<div class="thumbnail" style="background-image: url('+$href+')"></div>');
          }
        } else {
